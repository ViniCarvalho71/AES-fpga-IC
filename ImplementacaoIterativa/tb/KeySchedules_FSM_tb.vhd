...
entity KeySchedules_FSM_tb is
end entity KeySchedules_FSM_tb;

architecture sim of KeySchedules_FSM_tb is

    component KeySchedules_FSM is
        port(
...
begin
    ...
    begin
        ...
        -- Atribuir chave de teste FIPS-197
        ...
        -- Espera dinâmica pelo sinal done com timeout de segurança (ex: 2 us)
        wait until (done = '1') for 2 us;

        assert done = '1'
            report "ERRO: 'done' do KeySchedules_FSM nao subiu no tempo esperado!"
            severity error;

        -- Comparação das 11 round keys
                ...
                for j in 0 to 3 loop
                    ...
                    if keychain_out(r)(i, j) /= EXPECTED(r)(i, j) then
                        errors := errors + 1;
						  ...
						  if keychain_out(r)(i, j) /= EXPECTED(r)(i, j) then
								 report "ERRO: round=" & integer'image(r) &
								 " linha=" & integer'image(i) &
								 " coluna=" & integer'image(j) &
								 " esperado=" & integer'image(to_integer(unsigned(EXPECTED(r)(i, j)))) &
								 " obtido=" & integer'image(to_integer(unsigned(keychain_out(r)(i, j))))
								 severity error;
